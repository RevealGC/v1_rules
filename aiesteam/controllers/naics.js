...
const naicsLibBase ={
    naics_codes:{name: 'naics_codes', attributes:['naics','title'],
    dataType:['int', 'str'],
    where: {}, order: [[]]}
}
const analyticsLibBase ={
    analytics:{name: 'analytics', attributes:['naics_code','naics_title','companies','kaus','establishments', 
 ...
 * @param {*} next 
 */
    analyticsGetAll : (locals, req, res, next) => {
        ...
        if (filters["naics"]) { naics = filter.naics 
    ...
    if(locals.sortBy)
    var order = utils.buildOrderNoIncludes(locals, [['naics_code', 'ASC']]);//   utils.buildOrder(locals, [[]], ruLibBase);
        let { api_token, page, itemsPerPage, sortBy, sortDesc, filters, naics, title, filter } = locals
        let reqParams = "&debug=" + locals.DEBUG + "&itemsPerPage=" + itemsPerPage + "&naics=" + naics + "&filter=" + filter
        ...
        let whereClause = utils.buildWhereClause(where, naicsLibBase)
        where = whereClause.naics_codes.where
       

        // if (naics!= 0) {where.naics = { [Op.eq]:  naics}}
       
        // if (title.length > 0) {where.title = { [Op.like]: '%' + title + '%' }}
        
        // var include = []

        var attributes = ['naics', 'title']
   
      
      

        dbSeq.naics_codes.findAndCountAll({ 
            where,attributes, 
            // include, 
            limit: itemsPerPage, 
            offset,
